...
if (isset($_POST['btn_verify']) ) {
    ...
    $sql= "SELECT * FROM register WHERE matric_no = '$matricno' ";
    ...
    if (mysqli_num_rows($qr)==0) {
      echo "something when wrong please register again ";
      exit();
    }
    $record= mysqli_fetch_array($qr);

    if ($verifypin == $record['register_token']) {
      header('Location: ../newpasswordpage.php');
    }
    else
      header('Location: userverification.php?error=wrongcode');
      exit();
}
